...
int main(int argc, char* argv[]) {
    int sd, mirror, client, mirrorClient, portNumber, status = 0;
    struct sockaddr_in socAddr, mirrorAddr;
    ...
    // mirror also listens on the same port
    if ((mirror = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        fprintf(stderr, "Cannot create mirror socket\n");
        exit(1);
    }
    setsockopt(mirror, SOL_SOCKET, SO_REUSEPORT, &optval, sizeof(optval));
    bind(mirror, (struct sockaddr*)&socAddr, sizeof(socAddr));
    listen(mirror, MAX_CLIENTS);
    ...
    while (1) {
        ...
        // transfer connections to mirror after the first 4 clients
        if (status <= 4) {
            if (!fork()) {
        ...
        // mirror handles the next 4 clients
        if (status >= 4 && status < 8) {
            printf("Transferring client to mirror\n");
            send(mirror, &client, sizeof(client), 0);
            status++;
            close(client);
        }

        // alternating handling of clients between server and mirror
        if (status >= 8) {
